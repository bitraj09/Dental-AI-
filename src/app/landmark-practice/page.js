...
function evaluateWithMockAI(studentPoly, landmark) {
    const typical = landmark.typicalPolygon;
...
function getBounds(poly) {
    ...
    poly.forEach(p => { const x=p[0],y=p[1]; if(x<minX)minX=x; if(x>maxX)maxX=x; if(y<minY)minY=y; if(y>maxY)maxY=y; });
...
export default function LandmarkPracticePage() {
    ...
    const pickRandomLandmark = useCallback(() => {
    ...
    const handleCanvasClick = (e) => {
        ...
        if (drawingPoints.length >= 3) {
            ...
            const closeDist = Math.sqrt((xPct - first[0]) ** 2 + (yPct - first[1]) ** 2);
            if (closeDist < 0.025) {
                setIsPolygonClosed(true);
                return;
            }
        }
        setDrawingPoints(prev => [...prev, [xPct, yPct]]);
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (configuredModel === 'GOOGLE_AI') {
                ...
                // Try custom model, fall back to mock for landmarks
                try {
                    ...
                    if (res.ok) {
                        const data = await res.json();
                        // Use detections to cross-reference landmark area
                        const mockResult = evaluateWithMockAI(drawingPoints, currentLandmark);
                        evalResult = { ...mockResult, source: 'custom',
                            feedback: mockResult.feedback + (data.findings?.length ? ` (Custom model also detected ${data.findings.length} clinical finding(s) in this image.)` : '')
                        };
                    }
                } catch (err) {
    ...
    const handleNext = () => { pickRandomLandmark(); };
    ...
    const pointsStr = drawingPoints.map(p => `${p[0]*100},${p[1]*100}`).join(' ');

    // Build correct polygon overlay points
    const correctPolyPoints = result?.correctPolygon
        ? (Array.isArray(result.correctPolygon[0]) ? result.correctPolygon : []).map(p => `${p[0]*100},${p[1]*100}`).join(' ')
        : '';
                            ...
                            <div className={`${styles.imageContainer} ${!result && !isPolygonClosed ? styles.imageContainerDrawing : ''}`}>
                                ...
                                >
                                    ...
                                    {drawingPoints.length >= 2 && (
                                        <polyline points={pointsStr} fill={isPolygonClosed ? 'rgba(99,102,241,0.25)' : 'none'}
                                            stroke="#6366f1" strokeWidth="0.5" vectorEffect="non-scaling-stroke"
                                            strokeLinejoin="round" strokeDasharray={isPolygonClosed ? 'none' : '2,1'}
                                        />
                                    )}
